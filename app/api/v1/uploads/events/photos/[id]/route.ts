 ...
 * Handles PUT and DELETE requests for event photos
 */
import type { NextRequest } from 'next/server';
import { handleApiRoute } from '../../../../_helpers/api-route-handler';

// PUT /api/v1/uploads/events/photos/[id]
export async function PUT(
  ...
  try {
    ...
    if (contentType?.includes('text/html')) {
      console.error(`Received HTML response instead of JSON for event photo ${id} update`);
      return new Response(
        JSON.stringify({
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({
          error: errorData.error || 'Failed to update event photo',
    ...
    console.error(`Error in PUT /uploads/events/photos/${id}:`, error);
    return new Response(
      JSON.stringify({ error: 'Failed to update event photo' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
}

// DELETE /api/v1/uploads/events/photos/[id]
export async function DELETE(
  ...
  try {
    ...
    const contentType = response.headers.get('content-type');
    if (contentType?.includes('text/html')) {
      console.error(`Received HTML response instead of JSON for event photo ${id} delete`);
      return new Response(
        JSON.stringify({
          error: 'Invalid API endpoint or server error'
        }),
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({
          error: errorData.error || 'Failed to delete event photo',
          status: response.status
        }),
        {
          status: response.status,
    ...
    // Get the ID safely
    const id = await Promise.resolve(params).then(p => p.id).catch(() => 'unknown');
    console.error(`Error in DELETE /uploads/events/photos/${id}:`, error);
    return new Response(
      JSON.stringify({ error: 'Failed to delete event photo' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
}
