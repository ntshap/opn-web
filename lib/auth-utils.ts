  ...
  try {
    ...
    if (authToken) {
      ...
      if (authToken.length < 10) {
        console.warn('Auth check: Invalid token format in localStorage')
        return false
      }

      // Log authentication status for debugging
      console.log('Auth check: Valid auth_token found in localStorage')
      return true
    }
    ...
    if (token) {
      ...
      // Log authentication status for debugging
      console.log('Auth check: Valid token found in localStorage')
      return true
    }
